...
def _content_to_structured_images(content) -> tuple[Any, list]:
    """Convert only real image_url blocks into structured image entries."""
    if isinstance(content, str):
        return content, []
    if not isinstance(content, list):
        return ("" if content is None else str(content)), []
...
def _validate_rendered_image_alignment(processor, prompt_text: str, pil_images: list) -> None:
    """Reject rendered placeholder/image cardinality drift before processing."""
    ...
    if placeholder_count != len(pil_images):
        raise ValueError(
            ...
            f"rendered_placeholders={placeholder_count}, loaded_images={len(pil_images)}, "
...
def _messages_to_chat(state: ChatServerState, messages: list) -> tuple[list, list]:
    ...
    for m in messages:
        ...
        rendered = {"role": m.get("role"), "content": content}
...
async def _run_turn(state: ChatServerState, session: _Session, body: dict) -> tuple[str, str]:
    ...
    if not extends:
        ...
        chat, full_pil = _messages_to_chat(state, messages)
        full = state.processor.apply_chat_template(chat, tokenize=False, add_generation_prompt=True, **kwargs)
        _validate_rendered_image_alignment(state.processor, full, full_pil)
        ...
        # extends guarantees the prior messages are an exact prefix, so their
        # loaded images are the prefix of the full ordered image list.
        new_pil = full_pil[len(traj.pil_images) :]
