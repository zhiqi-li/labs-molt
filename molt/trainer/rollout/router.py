...
async def _execute_runner_with_policy_audit(
    *,
    ...
    When an engine version source is available, the two observations are
    authoritative. Without one, only an explicit scheduler lock guarantee can
    claim a frozen policy; otherwise the end version is intentionally unknown.
    """
    ...
    for traj in result if isinstance(result, list) else [result]:
        traj.extra_logs = dict(traj.extra_logs or {})
        traj.extra_logs["policy_version_start"] = start_version
