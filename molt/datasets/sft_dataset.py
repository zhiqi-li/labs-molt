...
class SFTDataset(Dataset):
    ...
    def _build_row(self, row) -> dict:
        ...
        return {
            ...
            # Keep nested schemas out of the Arrow feature inference path. Different
            # tools can legitimately have different JSON-schema shapes.
    ...
    def _normalize_openai_tool_calls(message: dict) -> dict:
        ...
        OpenAI conversations store ``function.arguments`` as a JSON string,
        while Transformers chat templates (including Qwen3.5) iterate it as a
        mapping. Preserve the dataset's wire format and normalize only the
        private copy rendered for training.
        """
